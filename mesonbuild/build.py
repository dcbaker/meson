...
class BuildTarget(Target):
    ...
    def process_compilers_late(self):
        ...
        if self.link_targets or self.link_whole_targets:
            def iter_compilers():
                yield from (l.compilers.items() for l in self.link_targets)
                yield from (l.compilers.items() for l in self.link_whole_targets)
            self.compilers.update(OrderedDict(
                sorted((i for i in itertools.chain.from_iterable(iter_compilers())
                        if i[0] in clike_langs),

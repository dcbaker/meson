...
manually set up the necissary link and compile args (and additional
dependencies as necessary).

For example, imagine a dependency called Foo, it uses an environment variable
called $FOO_ROOT to point to it's install root, which looks like this:
```txt
$FOOROOT
→ include/
→ lib/
```
To use Foo, you need it's include directory, and you need to link to
`lib/libfoo.$lib-end`.
...
This code will look for 'FOO_ROOT` in the environement, handle FOO_ROOT being
undefined gracefully, then set the it's `compile_args` and `link_args`
gracefully. It will also gracefully handle not finding the required lib
(hopefully that doesn't happen, but it could if say the lib is only static
and shared linking is requested).

There are a couple of thigns about this that still aren't ideal. For one, we
...
BOOST_ROOT is handled). So now that becomes:
...
means that users can override this in their machine files, and finally if
that environment variables changes durring a meson reconfigure meson won't
re-read it.

We can still make this better by handling the `static` keyword
```python
class FooSystemDependency(ExternalDependency):
...
Now we meson will figure out wither it should be finding `libfoo.so` or
`libfoo.a` (or the platform specific names). Things are looking pretty good
now, so it can be added to the `packages` dict below
```python
packages.udate({
...
or only inlucded in very recent versions? We can use the `DependencyFactory`
class:
```python
...
dependency, and use the `FooSystemDependency` as well. You would then add the
`foo_factory` to packages instead
```python
packages.udate({
    'foo': foo_factory,
})
...
number of examples in this pacakge.

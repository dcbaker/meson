...
''')
    ...
    def should_use_dyndeps_for_target(self, target: 'build.BuildTarget') -> bool:
        ...
        if cpp.get_id() != 'msvc':
            return False
        cppversion = self.get_target_option(target, OptionKey('cpp_std', machine=target.for_machine), str)
    ...
    def generate_vala_compile(self, target: build.BuildTarget) -> \
        ...
        args += valac.get_colorout_args(self.get_target_option(target, 'b_colorout', str))
    ...
    def generate_cython_transpile(self, target: build.BuildTarget) -> \
        ...
        ext = self.get_target_option(target, OptionKey('cython_language', machine=target.for_machine), str)
    ...
    def generate_rust_target(self, target: build.BuildTarget) -> None:
        ...
        if not isinstance(target, build.StaticLibrary):
            try:
                buildtype = self.get_target_option(target, 'buildtype', str)
                crt = self.get_target_option(target, 'b_vscrt', str)
...
https://gcc.gnu.org/bugzilla/show_bug.cgi?id=47485'''))
    ...
    def generate_link(self, target: build.BuildTarget, outname, obj_list, linker: T.Union['Compiler', 'StaticLinker'], extra_args=None, stdlib_args=None):
        ...
        commands += linker.get_optimization_link_args(self.get_target_option(target, 'optimization', str))
        # Add /DEBUG and the pdb filename when using MSVC
        if self.get_target_option(target, 'debug', bool):

...
class Vs2010Backend(backends.Backend):
    ...
    def get_args_defines_and_inc_dirs(self, target: build.BuildTarget, compiler, generated_files_include_dirs, proj_to_src_root, proj_to_src_dir, build_args):
        ...
        for lang in file_args.keys():
            file_args[lang] += self.get_target_option(target, OptionKey(f'{lang}_args', machine=target.for_machine), list)
    ...
    def add_non_makefile_vcxproj_elements(
        ...
        vscrt_type = self.get_target_option(target, 'b_vscrt', str)
        ...
        if 'cpp' in target.compilers:
            eh = self.get_target_option(target, OptionKey('cpp_eh', machine=target.for_machine), str)
            if eh == 'a':
        ...
        # Warning level
        warning_level = self.get_target_option(target, 'warning_level', str)
        ...
        if self.get_target_option(target, 'werror', bool):
        ...
        # /release
        if not self.get_target_option(target, 'debug', bool):
